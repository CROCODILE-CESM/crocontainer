...
The MOM6 jobs run a subset of the catalog, not all of it. `arctic_cap` is currently excluded: its forcing is produced fine (it passes `domain-sweep`) and MOM6 initializes on it, but the run goes unstable on the first coupled step -- most likely because a polar cap with flat bathymetry, no land, and open boundaries across the pole is not a physically sensible case, rather than anything wrong with CrocoDash.
