...
The MOM6 jobs run a subset of the catalog, not all of it. **`arctic_cap` is a known failure and is expected to be red.** Its forcing is produced fine (it passes `domain-sweep`) and MOM6 initializes on it cleanly, but the run goes unstable on the first coupled step -- NaN SSH on the boundary, then `FATAL: extreme surface values`. Most likely the test configuration rather than a CrocoDash bug: a polar cap with flat bathymetry, no land, and open boundaries across the pole is not a physically sensible case. It is kept in the matrix so it stays visible and so it will turn green by itself if that ever changes; it cannot block a PR, since these jobs only run on dispatch/schedule.
